...
public class AlarmController {
    ...
    @PostMapping(value = "/addAlarm", consumes = {"application/json"}, produces = {
            "application/json"})
    @ApiResponses(value = {@ApiResponse(responseCode = "201", description = "OK", content = {
            ...
            @ApiResponse(responseCode = "504", description = "Gateway Timeout", content = {
                    @Content(mediaType = "application/json")})})
    @Operation(summary = "Creating an alarm register", tags = "", method = "creatingAlarm")
    public ResponseEntity<AlarmOutputVO> creatingAlarm(
            @Valid @RequestBody AlarmInputVO alarmInputVO) throws Exception {
        log.debug("FLUJO - creatingAlarm");
        return new ResponseEntity<>(this.alarmService.addAlarm(alarmInputVO), HttpStatus.ACCEPTED);
    }
    ...
    public ResponseEntity<List<AlarmOutputVO>> getList
        ...
        return new ResponseEntity<>(this.alarmService.getListAlarmsByParams(key, idt, dateInit, dateFinish, severity), HttpStatus.ACCEPTED);
    }
